...
class JqGridJsonEncoder implements RequestedDataInterface
{
    ...
    public function encodeRequestedData(GridRepositoryInterface $Repository, $postedData)
    {
        ...
        if (empty($postedData['pivotRows'])) {
            $rows = $Repository->getRows($limit, $start, $sidx, $sord, $filters['rules'], $nodeId, $nodeLevel,
                $exporting);
                ...
                $Excel->sheet($postedData['name'], function ($Sheet) use ($rows, $postedData) {
                        ...
                        if (isset($model['formatter'])) {
                            switch ($model['formatter']) {
                                ...
                                case 'integer':
                                    ...
                                    array_push($numericColumns,
                                        isset($model['label']) ? $model['label'] : $model['name']);
                    ...
                    if (empty($groupHeaders)) {
                        ...
                        foreach ($groupHeaders as $index => $groupHeader) {
                            ...
                            $Sheet->mergeCells($this->numToLetter($columnsPositions[$groupHeader['startColumnName']],
                                    true) . '1:' . $this->numToLetter($columnsPositions[$groupHeader['startColumnName']] + $groupHeader['numberOfColumns'] - 1,
                                    true) . '1');
                        }
