...
class JqGridRender implements RenderInterface
{
    ...
    public function renderGrid($script = true, $createTableElement = true, $createPagerElement = true, $echo = true)
    {
        ...
        $script = 'var grid_selector = "#' . $this->gridId . '";var pager_selector = "#' . $this->options['pager'] . '";';
        $script .= 'var parent_column = jQuery(grid_selector).closest(\'[class*="col-"]\');';
        $script .= 'jQuery(window).on(\'resize.jqGrid\', function () {jQuery(grid_selector).jqGrid( \'setGridWidth\', parent_column.width() );});';
        if ($this->jqPivot) {
            $mtype = $this->options['mtype'];
            unset($this->options['colModel'], $this->options['mtype'], $this->options['datatype']);
            $script .= 'jQuery(grid_selector).jqGrid("jqPivot", "' . $this->options['url'] . '", ' . json_encode($this->pivotOptions) . ', ' . json_encode($this->options) . ', {async : false, type: "' . $mtype . '"});';
            $script .= 'jQuery(grid_selector).navGrid("#' . $this->options['pager'] . '", '
                . json_encode($this->navigatorOptions, JSON_FORCE_OBJECT) . ', '
                . json_encode($this->navigatorEditOptions, JSON_FORCE_OBJECT) . ', '
                . json_encode($this->navigatorAddOptions, JSON_FORCE_OBJECT) . ', '
                . json_encode($this->navigatorDeleteOptions, JSON_FORCE_OBJECT) . ', '
                . json_encode($this->navigatorSearchOptions, JSON_FORCE_OBJECT) . ', '
                . json_encode($this->navigatorViewOptions, JSON_FORCE_OBJECT) . ');';
        } else {
            $script .= 'jQuery(grid_selector).jqGrid(' . json_encode($this->options) . ');';
            $script .= 'jQuery(grid_selector).navGrid("#' . $this->options['pager'] . '", '
                . json_encode($this->navigatorOptions, JSON_FORCE_OBJECT) . ', '
                . json_encode($this->navigatorEditOptions, JSON_FORCE_OBJECT) . ', '
                . json_encode($this->navigatorAddOptions, JSON_FORCE_OBJECT) . ', '
        ...
        $script . 'jQuery(window).triggerHandler(\'resize.jqGrid\');';
        ...
        if (!empty($this->groupHeaderOptions)) {
            $script .= 'jQuery("#' . $this->gridId . '").jqGrid("setGroupHeaders", ' . json_encode($this->groupHeaderOptions) . ');';
        }
